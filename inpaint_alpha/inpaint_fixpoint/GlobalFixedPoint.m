...
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [u] = GlobalFixedPoint(u,z,alpha,beta,omega,GSiter) 
    [n,m] = size(u);
    h = 1;
    D = ZeroPadding(CoefficientD(u,beta),1);
    G = ZeroPadding(z,1);
    u = ZeroPadding(u,1);
    alpha = ZeroPadding(alpha,1);
    for iter=1:GSiter
        for i=2:n+1
            for j=2:m+1
            u(i,j) = (1-omega)*u(i,j)+omega*(G(i,j)+(alpha(i,j)/h^2)*...
                (D(i,j)*(u(i+1,j)+u(i,j+1))+D(i-1,j)*u(i-1,j)+D(i,j-1)*u(i,j-1)))/...
                (1+(alpha(i,j)/h^2)*(2*D(i,j)+D(i-1,j)+D(i,j-1)));
            end
        end
    end
    u = UnPadding(u,1);    
